...
- **완료:** STEP 0~3 (제품·정책 정리 / 네비게이션 설계 / 와이어프레임 확보 / React 스캐폴딩)
- **현재:** **STEP 4 시작 전** — Claude Design 와이어프레임 원본 반입·화면 매핑 대기
- **아직 안 함:** 공개 웹·앱 라우팅 전환, Pre-render 구성, i18n, 실제 UI 구현, API 연동, 네이티브 프로젝트
- **다음 행동 3가지:** (1) 와이어프레임 원본 안전 반입 → (2) 와이어프레임 화면 ↔ 현재 라우트 매핑 → (3) 공개 웹·앱 라우팅 아키텍처를 별도 PR로 준비
...
| 4    | 와이어프레임 반입·매핑  | 시안 원본을 저장소에 반입, 화면↔라우트 매핑 | 와이어프레임 자산, 매핑 문서     | STEP 3                   | 매핑 표 확정, 자산 반입         | 🔶 현재 | 자산·매핑 문서 PR                | 실 UI 구현·라우팅 전환     |
| 5    | 라우팅 경계 설계        | 공개 웹/앱 경계·`/app/*` 결정               | 라우팅 경계 설계(의사결정)       | STEP 4                   | 경계·라우트 계약 합의           | ⬜      | 라우팅 설계 PR                   | Pre-render 설정            |
...
- ✅ STEP 0~3 완료
- 🔶 STEP 4 시작 전
- 다음 작업: **Claude Design 원본 반입과 화면별 매핑**
- 라우팅·렌더링·i18n·실제 UI는 **아직 구현하지 않음**

**다음 실행 항목 (이 3가지만):**

1. [ ] Claude Design 원본(와이어프레임)을 저장소에 안전하게 반입
2. [ ] 와이어프레임 화면과 현재 라우트(`ROUTE_PATHS` 7종)를 매핑
3. [ ] 공개 웹·앱 라우팅 아키텍처 작업을 **별도 PR**로 준비
